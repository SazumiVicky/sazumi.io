...
footer.href = "https://www.sazumivicky.my.id/";

document.querySelector(".tombol").addEventListener('click', function () {
  Swal.fire("Hallo Cantik", "Aku ada pertanyaan nih buat kamu?", "question").then(function () {
    Swal.fire("Jawab jujur yah :(").then(function () {
      Swal.fire("Jangan Boong😠", "", "error").then(function () {

        const {
          value: name
        } = Swal.fire({
          title: 'Eh Nama Kamu Siapa Sih?',
          input: 'text',
          inputLabel: '',
          ...
          const pertanyaan = Swal.fire({
            ...
            if (result.isConfirmed) {
              Swal.fire(`${sender} juga sayang banget sama ${nama}`).then(function () {
                Swal.fire({
                  title: 'Coba Seberapa Sayang Kamu Sama Aku ',
                  icon: 'question',
                  input: 'range',
                  inputLabel: 'pili anka 1 - 100 ya',
                  inputAttributes: {
                    min: 1,
                    max: 100,
                    step: 1
                    ...
                    Swal.fire({
                      ...
                      if (result.isConfirmed) {
                        Swal.fire(`Huhu iya ${sender} juga kangen ${nama} :((`).then(function () {
                          Swal.fire('Ini Yang terakhir deh').then(function () {
                            Swal.fire('Coba klik ikon hati di paling bawah dong')
                          })
                        })
                      } else if (result.isDenied) {
                        Swal.fire('Kirain Kangen hehe', '', 'error').then(function () {
                          Swal.fire('Yaudah Deh Nanti Dulu yah')
                        })
                      }
                    })
                  })
              ...
              Swal.fire(`Yakin ga suka sama ${sender}?`, '', 'error').then(function () {
                Swal.fire('Hehe Kirain, Makasih Yah Bye')
              })
            }
          })
        })
